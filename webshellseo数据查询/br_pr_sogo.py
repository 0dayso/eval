...
def google_pr(url):
    try:
        ...
        PR_CH=hash_url(url)
        google="http://toolbarqueries.google.com.hk/tbr?client=navclient-auto&features=Rank:&q=info:%s&ch=%s"%(url,PR_CH)
        #print google
...
def sogo(url):  #搜狗SR
    try:
        #url="http://rank.ie.sogou.com/sogourank.php?ur=http://www.baidu.com/"%(url)
        sogo="http://rank.ie.sogou.com/sogourank.php?ur=http://%s/"%(url)
        data=url_post(sogo)
