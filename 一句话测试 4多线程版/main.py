            ...
            if yijuhua_cs(ASP_PHP,self.URL[0],self.URL[1]): #ASP还是PHP  ,URL地址 ，密码
                ...
                if ss==0:  #读取网页内容
                    ...
                    if sarr:
                        ...
                        NO+=1
                        time.sleep(1)
            print u"thread:%d 一句话总数%s条---成功%d条---失败%d条---还剩%d条需要测试"%(self.TH,yijuhuaZS,OK,NO,yijuhuaZS-(OK+NO))
        except:
            return 0
...
if __name__ == "__main__":
    ...
    threads = []  #线程
    for i in range(int(sys.argv[2])):  #nthreads=10  创建10个线程
        threads.append(C_Queue(i+1,))
